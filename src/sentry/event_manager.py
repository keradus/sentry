...
def _save_aggregate_new(
    ...
    # If we haven't, try again using the secondary config
        ...
        # If we still haven't found a group, ask Seer for a match (if enabled for the project)
            ...
            seer_matched_group = maybe_check_seer_for_matching_group(event, primary.hashes)

            if seer_matched_group:
                group_info = handle_existing_group_or_grouphash(
                    job, seer_matched_group, all_grouphashes, group_processing_kwargs
                )
